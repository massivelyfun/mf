...
(function() {
  ...
  root.rect.toGridBounds = function(rect) {
    return [
      {
        gridX: rect.point.x,
        gridY: rect.point.y
      }, {
        gridX: rect.point.x + rect.size.width,
        gridY: rect.point.y + rect.size.height
      }
    ];
  };
  ...
  root.rect.surfacePoints = function(rect, predicate) {
    var pts, x, y, _fn, _i, _j, _ref, _ref1;
    pts = [];
    for (y = _i = 0, _ref = Math.floor(rect.size.height); 0 <= _ref ? _i < _ref : _i > _ref; y = 0 <= _ref ? ++_i : --_i) {
      _fn = function(x, y) {
        if (predicate != null) {
      ...
      for (x = _j = 0, _ref1 = Math.floor(rect.size.width); 0 <= _ref1 ? _j < _ref1 : _j > _ref1; x = 0 <= _ref1 ? ++_j : --_j) {
        _fn(x, y);
      }
    }
    return pts;
  ...
  root.rect.intersects = function(rect1, rect2) {
    ...
    for (_i = 0, _len = _ref.length; _i < _len; _i++) {
      pt = _ref[_i];
      if (root.rect.contains(rect1, pt)) {
        return true;
      }
    }
    return false;
